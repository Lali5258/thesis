...
class Model:
	...
	def setupCNN(self):
		...
		for i in range(numLayers):
			...
			conv = tf.nn.conv2d(pool, filters=kernel, padding='SAME',  strides=(1,1,1,1))
	...
	def loadModel(self):
			...
			nepaliCharList = [
				...
				'ट', 'ठ', 'ड', 'ढ', 'ण', 'त', 'थ', 'द', 'ध', 'न',
				'प', 'फ', 'ब', 'भ', 'म', 'य', 'र', 'ल', 'व', 'श',
				'ष', 'स', 'ह', 'क्ष', 'त्र', 'ज्ञ', 'श्र', '१', '२', '३',
				'४', '५', '६', '७', '८', '९', '०'
			]
			model = Model(charList=nepaliCharList, decoderType=DecoderType.BestPath, mustRestore=False)
			# Step 3: Load Pre-trained Model (if available)
			pretrainedModelDir = 'C:/Users/User/PycharmProjects/pythonProject2/model/'
			latestPretrainedSnapshot = tf.train.latest_checkpoint(pretrainedModelDir)
			if latestPretrainedSnapshot:
				model.loadModel(latestPretrainedSnapshot)
			else:
				...
				for epoch in range(numEpochs):
					totalLoss = 0.0
					nepali_training_data = "C:/Users/User/PycharmProjects/pythonProject2/data/Nepdata/"
					numBatches = len(nepali_training_data) // batchSize

					for batchIdx in range(numBatches):
						startIdx = batchIdx * batchSize
						endIdx = (batchIdx + 1) * batchSize
						batch = nepali_training_data[
								startIdx:endIdx]  # Assuming you have already loaded the training data
